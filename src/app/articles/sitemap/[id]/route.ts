...
${entries.map(entry => `  <url>
    <loc>${entry.url}</loc>
    ${entry.lastModified ? `<lastmod>${entry.lastModified instanceof Date 
...
export async function GET(
  ...
  try {
    ...
    if (chunkId === 0) {
      ...
      console.log(`🏷️ Added ${categoryEntries.length} categories to sitemap chunk 0`)
    ...
    console.log(`✅ Articles sitemap chunk ${chunkId} generated with ${allEntries.length} total URLs`)
    ...
    console.error(`❌ Error generating articles sitemap chunk ${params.id}:`, error)
